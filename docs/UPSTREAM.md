...
| Bundle URL constants change | Not mitigated today. The base path (`/nges-portal/api/public/tracking-public/`) and the per-`ShipmentType` suffixes in `DETAIL_PATHS` are hardcoded from the snapshot devtools captured 2026-05-16. If DSV ships a bundle that renames any of those paths, requests will 404 and the HTML fallback will pick up the slack until the constants are refreshed. Runtime extraction from `runtime.*.js` + `main.*.js` was once listed as a Sprint 4 stretch but was never shipped; logged here so a future maintainer doesn't expect to find it. |
